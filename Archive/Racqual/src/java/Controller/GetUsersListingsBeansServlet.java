...
/**
 * This servlet is used to load/reload racquet listings from our database into
 * the session data. The servlet loads two ArrayLists of Listing beans into the
 * session data, one with data from all listings and the other with data from
 * the listings that have been authored by the user who is logged in, assuming a
 * user is logged in. If no user is logged in when the servlet is invoked, the
 * second of these servlets is left empty; generally, no exception/error is
 * incurred by invoking the servlet with no user logged in.
 *
 * @author The Dream Team
 * @version 2014/12/5
 */
@WebServlet(name = "GetUsersListingsServlet", urlPatterns
        = {
            "/GetUsersListings"
        })
public class GetUsersListingsBeansServlet extends HttpServlet {
    ...
    /**
     * This helper method does the actual updation of the Listing beans that sit
     * in the session data and represent the racquet listings in the database.
     * This is a cornerstone method in our program, as it is used by a variety
     * of servlets, including this one, to update the listing data in the
     * session data.
     *
     * @param request servlet request
     * @param response servlet response
    ...
    public void updateListingBeansInSessions(HttpServletRequest request,
        ...
        try {
            ...
            while (set.next()) {
                ...
                listingBean.setRacquet(racquetBean);
